...
var ycombo = function(ret, fileId, deps, config) {
    ...
    settings.compDomain = config.compDomain && config.compDomain;
...
//拼接组件方法
function genUrl(ids) {
    var url = settings.compDomain + '/components/??';
    var comboIds = [];
    var replaceContext = '';
    var file = fis.project.lookup(ids[0]).file;
    if(!file){
        log.error('can not find file named '+ids[0])
    }

    if (type(ids) === 'string') ids = ids.split(' ');

    if (settings.compDomain === void 0 && file.release) {
        var topName = file.release.split(file.id)[0];
    ...
    } else if(settings.compDomain === void 0 && !file.release){
        log.error('缺少合并组件CDN 或 组件输出路径');
    } else{
        each(ids, function(value, key) {
...
function genJsUrl(ids) {
    ...
    each(ids, function(id, i) {
        ...
        if(!file){
            log.error('can not find file named '+id)
        }
        var topName = file.release.split(file.id)[0];
        if(!comboIds[topName]){
            comboIds[topName] = [];
        }
        comboIds[topName].push(file.id);
    })
    each(comboIds, function(ids,key){
